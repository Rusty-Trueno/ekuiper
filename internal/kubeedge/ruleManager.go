...
var registry = &RuleRegistry{internal: make(map[string]*RuleState)}
...
func doStartRule(rs *RuleState, ruleProcessor *processor.RuleProcessor) error {
	...
	go func() {
		...
		case err := <-tp.Open():
			if err != nil {
				...
				logrus.Printf("closing rule %s for error: %v", rs.Name, err)
				tp.Cancel()
				rs.Triggered = false
			} else {
				rs.Triggered = false
				logrus.Printf("closing rule %s", rs.Name)
			}
		}
	}()
	return nil
...
func getAllRulesWithStatus(ruleProcessor *processor.RuleProcessor) ([]map[string]interface{}, error) {
...
func startRule(name string, ruleProcessor *processor.RuleProcessor) error {
	var rs *RuleState
	...
	err := doStartRule(rs, ruleProcessor)
	if err != nil {
		return err
	}
	return nil
...
func stopRule(name string, ruleProcessor *processor.RuleProcessor) (result string) {
...
func restartRule(name string, ruleProcessor *processor.RuleProcessor) error {
	stopRule(name, ruleProcessor)
	return startRule(name, ruleProcessor)
}

func recoverRule(name string, ruleProcessor *processor.RuleProcessor) string {
	...
	err = startRule(name, ruleProcessor)
	if err != nil {
		return fmt.Sprintf("%v", err)
	}
